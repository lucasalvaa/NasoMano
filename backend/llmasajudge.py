...
JUDGE_INSTRUCTIONS = """
You are an expert prompt annotator. Analyze the user's prompt and extract 5 specific dimensions.
Some prompts may contain code snippets. You must not take any strings or code comments into account under any circumstances!

DEFINITIONS:
1. ROLE SUPPRESSION (is_role_assigned): True if the prompt explicitly assigns a persona/role (e.g., "Act as...", "You are an expert...").
2. REASONING SUPPRESSION (reasoning_score): True if the prompt is structured in such a way as to allow the LLM to reason through a Chain of Thought  (e.g., "Let's think step by step", "First... Then..."), 0.0 if no CoT is present.
...
async def evaluate_prompt_with_llm(client: AsyncClient, user_prompt: str, semaphore: asyncio.Semaphore) -> dict:
    if not user_prompt:
        return {
            "is_role_assigned": None, "reasoning_score": None,
            "self_reflection_present": None, "structure_specified": None,
            "examples_count": None, "error": "Empty prompt"
        }
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                return {
                    ...
                    "reasoning_score": result_json.get("reasoning_score"),
    ...
    with tqdm(total=len(user_prompts), desc="Valutazione in corso") as pbar:
        for i in range(0, len(user_prompts), CHECKPOINT_INTERVAL):
            ...
            temp_df = pd.DataFrame({
                ...
                "reasoning_score": [res.get("reasoning_score") for res in results],
...
def main():
    ...
    try:
        ...
        for col in ["is_role_assigned", "reasoning_score", "self_reflection_present", "structure_specified",
                    "examples_count", "error"]:
    ...
    if len(final_df) > 0:
        ...
        print(f"Reasoning (True): {(valid_rows['reasoning_score'] == True).sum()}")
